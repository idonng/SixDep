...
public class AuthFilter implements Filter {  
    ...
    public void doFilter(ServletRequest request, ServletResponse response,  
		...
		if (requestURI.startsWith("/GetOpenId.do")||requestURI.startsWith("/page/jsp/loginByVerification.jsp")) {
	//	if ("/GetOpenId.do".equals(requestURI)) {
...
//			}
		...
		// 如果第一次请求不为登录页面,则进行检查用session内容,如果为登录页面就不去检查.
		else if (!"/page/login.jsp".equals(requestURI)
				&& !"/page/login.do".equals(requestURI)
				&& !"/".equals(requestURI)) {
			// 取得session. 如果没有session则自动会创建一个,
			// 我们用false表示没有取得到session则设置为session为空.
			HttpSession session = req.getSession(false);
			...
			if (session == null || session.getAttribute("user") == null) {
				...
				out.println("<html>");
				out.println("<script>");
				out.println("window.open ('"+req.getContextPath()+"/page/login.jsp','_top')");
				out.println("</script>");
				out.println("</html>");
